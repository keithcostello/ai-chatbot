...
WORKFLOW:
  ...
  phases:
    ...
    - phase: 6
      ...
      pm_duties:
        - "Verify all 8 docs updated in steertrue/docs/"
      ...
      lessons_learned_requirement:
        file: ".claude/sprints/mlaia/LESSONS_LEARNED.aipl"
...
# ═══════════════════════════════════════════════════════════════
# VERIFICATION AGENT INTERACTIONS (V4.0)
# ═══════════════════════════════════════════════════════════════

VERIFICATION_AGENTS:
  description: "PM interacts with fresh-context verification agents"

  test_verifier_interaction:
    trigger: "Test Verifier returns BLOCKED with blocker_type=spec_missing"
    pm_action: |
      1. Test Verifier couldn't run tests due to unclear test spec
      2. PM must clarify test commands in CONTEXT.md
      3. PM updates test specification
      4. Orchestrator re-runs Test Verifier

  human_uat_ai_interaction:
    trigger: "Human-UAT AI returns FAIL"
    pm_action: |
      1. Human-UAT AI found issue PM missed
      2. PM receives failure reason from orchestrator
      3. PM routes failure to DEV
      4. DEV enters FIX_REVIEW protocol

  escalation_validator_arbitration:
    trigger: "DEV disputes Escalation Validator ANSWER_EXISTS"
    pm_action: |
      1. Orchestrator presents both sides to human
      2. Human decides: DEV or Validator correct
      3. If DEV correct → PM receives valid block, provides guidance
      4. If Validator correct → DEV re-reads cited file:line
...
BRANCH_POLICY:
  ...
  infrastructure_handshake:
    ...
    source_of_truth: "steertrue/docs/INFRASTRUCTURE.md"

    step_1_context_md:
      action: "Create CONTEXT.md from template"
      template: ".claude/sprints/mlaia/CONTEXT_TEMPLATE.md"
      output: ".claude/sprints/mlaia/sprint-X.Y.Z/CONTEXT.md"
      max_lines: 30
      must_contain:
