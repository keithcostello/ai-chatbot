...
Task(subagent_type="pm-agent", prompt="""
proof_nonce: {nonce}
...
Delegate to dev-executor to submit READY:
```
# Generate nonce
nonce = secrets.token_hex(4)
...
Delegate to pm-agent to review READY:
```
# Generate nonce
nonce = secrets.token_hex(4)
...
Log Test Verifier result to `agent-log.md`

### Step 4.5a: Truth Audit (Test Verifier) - MANDATORY

**CRITICAL:** Audit test-verifier execution before proceeding.
...
Task(subagent_type="truth-auditor", model="sonnet", prompt=f"""
Audit test-verifier execution.

Sprint: [sprint-id]
Last action: test-verifier returned [PASS/FAIL/BLOCKED]
Agent-log entry timestamp: {timestamp}

Verify with YOUR OWN tool calls:
1. Is timestamp real? (has milliseconds, not round number)
2. Did test-verifier actually run pytest? (grep checkpoint for pytest output)
3. Does agent-log.md show test-verifier entry?
...
If VIOLATION → HALT_SPRINT immediately
...
**Step 5.7.** If Human-UAT AI returns:
...
**CRITICAL:** Orchestrator must log Human-UAT AI result to agent-log.md BEFORE proceeding to Phase 6. Missing log entry = process violation.

**Step 5.7a.** Truth Audit (Human-UAT AI) - MANDATORY

**CRITICAL:** Audit human-uat-executor before proceeding to Phase 6.
...
Task(subagent_type="truth-auditor", model="sonnet", prompt=f"""
Audit human-uat-executor (Layer 2.5).

Sprint: [sprint-id]
Last action: human-uat-executor returned [PASS/FAIL/ASK]
Agent-log entry timestamp: {timestamp}

Verify with YOUR OWN tool calls:
1. Is timestamp real? (has milliseconds, not round number)
2. Did human-uat-executor actually run Bash commands? (not just review evidence)
3. Does agent-log.md show human-uat-executor entry?
4. Is test output ACTUAL (pasted) or SUMMARIZED?
5. Can you verify the endpoint is actually working? (run a health check yourself)

Return: CLEAN or VIOLATION with evidence.
""")
```

If VIOLATION → HALT_SPRINT immediately (do NOT proceed to Phase 6)
...
### Step 6.2: DEV Documentation Update
...
1. Delegate to pm-agent for final review and grade
2. Update `state.md` → phase=GRADED
3. Go to Phase 7
...
### Step 7.2a: FINAL Truth Audit (Pre-Merge) - MANDATORY
...
1. TIMESTAMPS: Read agent-log.md, check ALL timestamps have milliseconds (not round numbers)
2. CHECKPOINTS: Verify all checkpoint files exist (1-6)
3. HUMAN-UAT: Verify agent-log.md has human-uat-executor entry with PASS
4. BRANCH: Run git branch --show-current, verify matches CONTEXT.md
5. STATE: Verify state.md shows phase=MERGE_PENDING
6. EVIDENCE: Grep checkpoints for actual pytest output (not summarized)
7. COMMITS: Run git log --oneline -5, verify commits exist

This is the LAST GATE before merge. Any violation here means the entire sprint may be compromised.
