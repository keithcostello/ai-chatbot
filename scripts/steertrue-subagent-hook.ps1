...
# ============================================================================
# Session ID Generation (Option A: Unique per subagent)
# ============================================================================
# Generate unique session_id for each subagent to isolate decay state
$parent_session = $null
if ($inputJson.session_id) {
    $parent_session = $inputJson.session_id
} elseif (Test-Path $SESSION_FILE) {
    try {
        $sessionData = Get-Content $SESSION_FILE -Raw | ConvertFrom-Json
        $parent_session = $sessionData.session_id
    } catch {
        $parent_session = "unknown"
    }
}
if (-not $parent_session) { $parent_session = "unknown" }

# Create unique subagent session: parent-type-timestamp
$timestamp = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
$session_id = "$parent_session-$subagent_type-$timestamp"
...
try {
    ...
    # Add session_id (isolates decay state per subagent)
    if ($session_id) {
...
"@
    ...
    # Log error to file for diagnosis, but don't block subagent execution
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $logEntry = "$timestamp | SUBAGENT_HOOK_FAIL | session=$session_id | type=$subagent_type | error=$_"
    $logEntry | Out-File -Append "$env:TEMP\steertrue-hook-errors.log"
